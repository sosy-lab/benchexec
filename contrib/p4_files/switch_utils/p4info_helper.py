...
class P4InfoHelper(object):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            "%r object has no attribute %r (check your P4Info)" % (self.__class__, attr)
        )
        ...
        for mf in t.match_fields:
            if name is not None:
                ...
                if mf.id == id:
                    return mf
        raise AttributeError(
            "%r has no match field %r (check your P4Info)"
    ...
    def build_act_prof_member(
        self, act_prof_name, action_name, action_params=None, member_id=None
    ):
