...
// --------------------rendering-----------------------------
        ...
        if (this.state.isValue) {
            ...
            this.props.tools[index].columns.forEach(column => {
                if (!(column.type.name === "status" || column.type.name === "text") && column.isVisible) {
                    this.renderData(this.props.table, column.display_title, index, column.display_title);
                }
            })
        }
    }
    ...
    renderData = (rows, column, tool, field) => {
        ...
        if(!this.state.isValue || index >= 0) {
            const relevantRows = this.state.correct ? rows.filter(rs => rs.results[tool].category === "correct") : rows;

            arrayY = relevantRows.map(runSet => [runSet.results[tool].values[index].original, runSet.short_filename]);
    ...
    sortArray = (array, column) => {
        ...
        return (this.state.isValue && ['text', 'status'].includes(currentValue.type.name))
    ...
    renderColumns = () => {
        this.props.tools.forEach(tool => {
            tool.columns.forEach(column => {
                if (column.isVisible && this.possibleValues.findIndex(value => value.display_title === column.display_title) < 0) {
...
// ------------------------handeling----------------------------
    ...
    handleColumn = (ev) => {
        this.setState({
            ...
            isValue: this.props.tools
                            .map(tool => (tool.columns.findIndex(value => value.display_title === ev.target.value) >= 0))
    ...
    handleType = () => {
        ...
        return (this.state.isValue && ((type && type.name === "text") || (type && type.name === "status"))) 
            ? 'ordinal' 
            : this.state.linear ? 'linear': 'log';
    }
