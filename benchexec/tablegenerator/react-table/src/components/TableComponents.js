...
export const StandardColumnHeader = ({column, title = "Click here to sort. Hold shift to multi-sort", ...other}) => (
...
export const StandardCell = ({cell, href = cell.value.href, toggleLinkOverlay, force = false, ...other}) => {
  ...
  if (href) {
    return (
      <a href={href} onClick={ev => toggleLinkOverlay(ev, href)} dangerouslySetInnerHTML={html ? {__html: html} : undefined} {...other}>
        {raw}
      </a>
    );
  }
  return (
    <div dangerouslySetInnerHTML={html ? {__html: html} : undefined} {...other}>
      {raw}
    </div>
  );
};
...
export const createRunSetColumns = (runSet, runSetIdx, createColumn) => [
  SeparatorColumn,
  {
    ...
    columns: runSet.columns.map((column, columnIdx) => createColumn(runSetIdx, column, columnIdx)),
  },
];
