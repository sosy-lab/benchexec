...
class Tool(benchexec.tools.template.BaseTool):
    ...
    def cmdline(self, executable, options, tasks, propertyfile=None, rlimits={}):
        if SOFTTIMELIMIT in rlimits:
            if "--timelimit" in options:
                ...
                options += ["--timelimit", str(rlimits[SOFTTIMELIMIT])]  # benchmark-xml uses seconds as unit
        ...
        if ("-stats" not in options):
            options = options + ["--cpa-args", "-stats"]

        spec = ["--cpa-args", "-spec " + propertyfile] if propertyfile is not None else []
    ...
    def determine_result(self, returncode, returnsignal, output, isTimeout):
        ...
        for line in output:
            ...
            elif 'Invalid configuration: ' in line and not status:
                if 'Cannot parse witness' in line:
                    status = result.RESULT_ERROR
                    status += ' (invalid witness file)'
            elif 'For your information: CPAchecker is currently hanging at' in line and not status and isTimeout:
                status = 'TIMEOUT'
